...
function adminmodel() {
    ...
    //     db.collection("reg").find({"unm":userDet.unm,"pwd":userDet.pwd}).toArray((err,result)=>{
    //         if(err)
    //         console.log(err)
    //         else
    //         cb(result)
    ...
    this.fatchusers=(userDetails,cb)=>{
        db.collection('reg').find({'status':1}).toArray((err,result)=>{
            if(result.length>0)
            {
                cb(result)
            }else{
